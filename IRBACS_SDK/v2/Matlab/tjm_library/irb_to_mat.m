...
for index=0:frameCount-1
    ...
    if ~bres
        ...
        if index == 0
            start_time =['DAQstart_',int2str(timestamp.wDay),...
              '.',int2str(timestamp.wMonth),'.',...
              int2str(timestamp.wYear),'_',...
              int2str(timestamp.wHour),'.',...
              int2str(timestamp.wMinute),'.',...
              int2str(timestamp.wSecond),'.',...
              sprintf('%03d',timestamp.wMilliSeconds)];
        end
...
%Add properties from TIR Camera
[~, IRmatrix.properties.epsilon] = irbacs_GetParam(glbIRBFileHnd,ParamConst.pavEpsilon);
[~, IRmatrix.properties.camTemp] = irbacs_GetParam(glbIRBFileHnd,ParamConst.);
if length( save_dir ) > 1
    ...
    save(['IR_data_',start_time],'-struct', 'IRmatrix', '-v7.3') ;
